...
4.  **조치 모드 (Remediate)**
    *   취약점을 점검하고, 발견된 모든 관련 정책을 자동으로 '사용(1)'으로 설정합니다.
    *   조치 전에 현재 설정을 `W-78.backup.json` 파일로 백업합니다.
    *   실행 명령:
        ```powershell
...
5.  **복원 모드 (Restore)**
    *   이전에 `Remediate` 모드에서 백업된 `W-78.backup.json` 파일을 사용하여 설정을 원래 상태로 복원합니다.
...
6.  모든 실행 과정은 `KISA_LOG/W-78.log`에 기록되며, 최종 결과는 `KISA_RESULT/W-78.json` 파일에 저장됩니다.
